	...
	if (blm != null) {
	  ...
	  for (int y = absStartY; y < absEndY; y++) {
		...
		for (int x = absStartX; x < absEndX; x++) {
		  ...
		  canvas.drawRect(pixelWidth * clippedX + PIXEL_PADDING, pixelHeight
			  * clippedY + PIXEL_PADDING, pixelWidth * (clippedX + 1)
			  - PIXEL_PADDING, pixelHeight * (clippedY + 1) - PIXEL_PADDING,
			  paint);
		}
	  }
	}
