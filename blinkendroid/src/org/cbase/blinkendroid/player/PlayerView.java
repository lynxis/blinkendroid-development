...
public class PlayerView extends ClippableView implements Runnable, BlinkenView {
    ...
    public PlayerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        screenLayout = context.getResources().getConfiguration().screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK;
        System.out.println("ScreenLayout is : "+ screenLayout);
    }
        ...
        if (blm != null) {
            ...
            for (int y = absStartY; y < absEndY; y++) {
                ...
                for (int x = absStartX; x < absEndX; x++) {
                    ...
                    //GalaxyTab
                    if (Configuration.SCREENLAYOUT_SIZE_LARGE == screenLayout) {
                        canvas.drawRect(pixelWidth * clippedY + PIXEL_PADDING, pixelHeight * clippedX + PIXEL_PADDING,
                                pixelWidth * (clippedY + 1) - PIXEL_PADDING, pixelHeight * (clippedX + 1)
                                        - PIXEL_PADDING, paint);
                    } else {
                        //other devices
                        canvas.drawRect(pixelWidth * clippedX + PIXEL_PADDING, pixelHeight * clippedY + PIXEL_PADDING,
                                pixelWidth * (clippedX + 1) - PIXEL_PADDING, pixelHeight * (clippedY + 1)
                                        - PIXEL_PADDING, paint);
