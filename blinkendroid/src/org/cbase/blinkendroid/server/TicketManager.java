...
public class TicketManager implements IPeerHandler, ConnectionListener {
	...
	try {
	    socket = new DatagramSocket(Constants.BROADCAST_ANNOUCEMENT_SERVER_TICKET_PORT);
	...
	if (clients < maxClients || tickets.contains(ip)) {
		...
		socket.send(packet);
		Log.d(Constants.LOG_TAG, "send Ticket for " + name + " " + ip);
		if (!tickets.contains(ip)) {
		    clients++;
		    tickets.add(ip);
		} else {
		    Log.d(Constants.LOG_TAG, "reset sent ticket for " + name + " " + ip);
		}
	    } catch (Exception e) {
