...
public class PlayerManager implements ConnectionListener {
	...
	for (int i = 0; i < maxY; i++) {
		...
		if (mMatrixClients[i][j] != null) {
		    ...
		    if (clipAll)
			playerClient.getBlinkenProtocol().clip(
				playerClient.startX, playerClient.startY,
	...
	if (videoServer == null) {
		...
		for (int j = 0; j < maxX; j++) {
		    if (null != mMatrixClients[i][j]) {
			Log.i(Constants.LOG_TAG, "play PlayerClient " + j + ":"
				+ i + " " + filename);
			mMatrixClients[i][j].getBlinkenProtocol().play(
				startTime, filename);
		    }
		}
	    }
	...
	for (int i = 0; i < maxY; i++) {
	    for (int j = 0; j < maxX; j++) {
		if (null != mMatrixClients[i][j]) {
		    if (mMatrixClients[i][j].getClientSocketAddress().equals(
			    socketAddr)) {
	...
	if (client != null) {
		...
		if (ConnectionState.Command.SYN.ordinal() == data) {
			...
			client = addClient(new ClientSocket(
				blinkendroidprotocol, socketAddr));
			protoData.rewind();
