	  ...
	  if (null == movieFile) {
		...
		if (null != movie && movie.exists()) {
		  ...
		  try {
			...
			is.close();
			Log.d(Constants.LOG_TAG, "send movie bytes " + movie.length());
			writeLong(out, movie.length());
		  } catch (IOException ioe) {
			Log.e(Constants.LOG_TAG, "sending movie failed", ioe);
		  }
		} else {
		  Log.e(Constants.LOG_TAG, "movie not found" + movieFile);
		}
	  }
  ...
  class ReceiverThread extends Thread {
	...
	public void run() {
	  ...
	  try {
		...
		Log.e(Constants.LOG_TAG, "Socket closed", e);
	  } catch (IOException e) {
		Log.e(Constants.LOG_TAG, "InputThread fucked", e);
		e.printStackTrace();
	  }
	  Log.d(Constants.LOG_TAG, "InputThread ended!!!!!!!");
	...
	public void shutdown() {
	  Log.d(Constants.LOG_TAG, " ReceiverThread shutdown start");
	  running = false;
	  interrupt();
	  Log.d(Constants.LOG_TAG, " ReceiverThread shutdown interrupted");
	  try {
		join();
	  } catch (InterruptedException e) {
		Log.e(Constants.LOG_TAG, "ReceiverThread join failed", e);
		e.printStackTrace();
	  }
	  Log.i(Constants.LOG_TAG, "ReceiverThread shutdown joined & end");
	}
  }
}