...
public class BlinkendroidClientProtocol extends BlinkendroidProtocol implements
	CommandHandler {
	...
	if (mListener != null) {
		...
		BLM blm = BlinkendroidVideoClientProtocol
			.receiveMovie((InetSocketAddress) from);
		// if length == 0 play default
		/*
		 * if (length == 0) { } // else read BLM else { blm =
		 * parser.parseBBMZ(in, length); // while (in.read(buffer) !=
		 * -1) { // inputLine= ByteBuffer.wrap(buffer).getInt(); // if
		 * (!running) // fast exit // break; // } long length2 =
		 * in.getLong(); System.out.println("play length1 " + length +
		 * " length2:" + length2); }
		 */
